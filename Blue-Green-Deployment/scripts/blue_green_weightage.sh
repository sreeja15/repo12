...
azure_resource_group="bgdeploymentnew"
azure_tm_profile_name="bg-test-new"
...
tm_show_blue_endpoint() {
  ...
  blue_weightage=$(az network traffic-manager endpoint show --resource-group "${azure_resource_group}" --profile-name "${azure_tm_profile_name}" --name "${azure_tm_blue_endpoint}" --type "${azure_tm_endpoint_type}" --query 'weight' -o tsv)
...
tm_show_green_endpoint() {
  ...
  green_weightage=$(az network traffic-manager endpoint show --resource-group "${azure_resource_group}" --profile-name "${azure_tm_profile_name}" --name "${azure_tm_green_endpoint}" --type "${azure_tm_endpoint_type}" --query 'weight' -o tsv)
